...
def collect_metrics(y_probs_test, y_true_test, y_pred_test, sample_indices_test,
	...
	dataset_index = pd.read_csv("unique_unseen_epilepsy_corpus_window_index.csv")
...
if __name__ == "__main__":
	...
	MASTER_DATASET_INDEX = pd.read_csv("unique_unseen_epilepsy_corpus_window_index.csv")
	...
	X = np.load("../data/saved_numpy_arrays/X_psd_unique_unseen_epilepsy_corpus.npy", allow_pickle=True)
	y = np.load("../data/saved_numpy_arrays/y_psd_unique_unseen_epilepsy_corpus.npy", allow_pickle=True)
	...
	assert (label_mapping == ["diseased", "healthy"]).all() == True
